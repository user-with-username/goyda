...
pub fn build_listener_class(spec: &ListenerSpec) -> Vec<u8> {
    ...
    let methods: Vec<(u16, u16)> = spec
        .methods
        .iter()
        .map(|m| (cp.utf8(m.name), cp.utf8(m.descriptor)))
        .collect();
    ...
    {
        let mut w = |v: u32, size: usize| {
            out.extend_from_slice(&v.to_be_bytes()[4 - size..]);
        };
        ...
        w(0x0001, 2);
        w(init_name as u32, 2);
        w(init_desc as u32, 2);
        w(1, 2);
        w(attr_code as u32, 2);
        w(22, 4);
        w(3, 2);
        w(3, 2);
        w(10, 4);

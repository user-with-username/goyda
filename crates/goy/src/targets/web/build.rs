...
pub fn build_web_app(ctx: &BuildContext) -> Result<PathBuf> {
    ...
    copy_dir_recursive(
        &ctx.manifest_dir.join("assets"),
        &layout.dist_dir().join("assets"),
    )
    .context("Failed to copy the assets/ directory into the web build")?;
    s.ok();

    let s = term::spinner_step("package wasm");
    let bound = run_wasm_bindgen(
        &env.wasm_bindgen_bin,
        &wasm_path,
        layout.dist_dir(),
        &ctx.lib_name,
    );
...
fn compile_to_wasm(ctx: &BuildContext) -> Result<()> {
    ...
    run_command_quiet(
        &mut cmd,
        "building the Rust library for wasm32 via cargo build failed",
    )
}
...
fn write_index_html(layout: &WebAppLayout, app_name: &str, out_name: &str) -> Result<()> {
    ...
    std::fs::write(layout.dist_dir().join("index.html"), rendered)

...
"#;
    ...
    if workspace_lockfile.exists() {
        fs::copy(
            workspace_lockfile,
            layout.workspace_dir().join("Cargo.lock"),
        )
...
fn compile_workspace(
    ...
    cmd.current_dir(layout.workspace_dir())
        .args(&args)
...
fn assemble_bin_dir(
    ...
    fs::copy(&host_exe, layout.final_exe())
        .context("Failed to copy the host .exe into the windows build output")?;
    ...
    fs::copy(&goyda_dll, layout.bin_dir().join("goyda.dll"))
        .context("Failed to copy goyda.dll into the windows build output")?;

    if let Some(std_dll) = find_std_dylib(&ctx.target_triple)? {
        let dest = layout.bin_dir().join(std_dll.file_name().unwrap());
        if !dest.exists() {
            fs::copy(&std_dll, &dest)
                .context("Failed to copy the dynamic std runtime into the windows build output")?;
        }
    }
    ...
    if include_generation_zero {
        ...
        fs::copy(&built_dll, &gen0)
            .context("Failed to copy the consumer dylib into the windows build output")?;
    }

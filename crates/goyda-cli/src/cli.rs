...
mod tests {
    ...
    fn new_parses_name_and_defaults() {
        ...
        match cli.cmd {
            Cmd::New { name, template, manifest_dir } => {
    ...
    fn new_accepts_explicit_template_and_manifest_dir() {
        let cli = Cli::try_parse_from([
            "goy", "new", "myapp", "--template", "blank", "--manifest-dir", "/tmp/x",
        ]).unwrap();
        match cli.cmd {
            Cmd::New { template, manifest_dir, .. } => {
    ...
    fn init_defaults_template_and_manifest_dir() {
        ...
        match cli.cmd {
            Cmd::Init { template, manifest_dir } => {
                assert_eq!(template, "counter");
                assert_eq!(manifest_dir, PathBuf::from("."));
            }
    ...
    fn run_requires_a_platform() {
        ...
        match cli.cmd {
            Cmd::Run { platform, target, release, .. } => {
    ...
    fn run_accepts_an_explicit_target_triple() {
        let cli = Cli::try_parse_from([
            "goy", "run", "android", "--target", "aarch64-linux-android",
        ]).unwrap();
        match cli.cmd {

...
impl PlatformTarget for AndroidTarget {
    ...
    fn build(&self, ctx: &BuildContext) -> Result<PathBuf> {
        ...
        templates::generate_assets_from_templates(&layout, &ctx.crate_name, &ctx.lib_name, ctx.release)?;
    ...
    fn full_reset(&self, ctx: &BuildContext) -> Result<()> {
        ...
        let _ = Command::new(&adb_path).args(&["uninstall", &package_name]).output();
        Ok(())
    }
    ...
    fn quick_reload(&self, ctx: &BuildContext) -> Result<bool> {
        ...
        if !built_so_path.exists() {
            bail!("Compiled consumer library not found at: {:?}", built_so_path);
        }
...
fn push_next_generation(adb_path: &Path, package: &str, local_so: &Path) -> Result<String> {
    ...
    let _ = Command::new(adb_path).args(&["shell", "run-as", package, "mkdir", "-p", "files"]).output();
    ...
    let next_generation = listing
        .lines()
        .filter_map(|name| name.trim().strip_prefix("app_gen")?.strip_suffix(".so")?.parse::<u32>().ok())
        .max()
        .map_or(0, |m| m + 1);
    ...
    run_adb_quiet(
        adb_path,
        &["push", local_so.to_str().context("Invalid .so path")?, &tmp_path],
    ...
    let _ = Command::new(adb_path).args(&["shell", "rm", &tmp_path]).output();
...
fn send_hot_swap(adb_path: &Path, package: &str, device_path: &str) -> Result<()> {
    ...
    run_adb_quiet(
        adb_path,
        &["shell", "am", "broadcast", "-n", &receiver_component, "-a", &action, "--es", "path", device_path],

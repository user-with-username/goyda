...
pub fn build_native_library(ctx: &BuildContext) -> Result<()> {
    if is_windows_host() {
        ...
        let mut cmd = Command::new("cargo");
        cmd.current_dir(&ctx.manifest_dir).args(&args).env("RUSTFLAGS", PREFER_DYNAMIC);
...
pub fn copy_native_library(ctx: &BuildContext, layout: &AndroidAppLayout) -> Result<()> {
    let profile_dir = ctx.target_directory.join(&ctx.target_triple).join(ctx.profile_dir());
    ...
    if let Some(std_so_path) = find_std_dylib(&ctx.target_triple)? {
        ...
        std::fs::copy(&std_so_path, &dest).context("Failed to copy the dynamic std runtime into the APK layout")?;
    }

...
fn open_browser(url: &str) {
    ...
    if result.map(|s| !s.success()).unwrap_or(true) {
        term::info(&format!("Could not open a browser automatically - open {url} manually."));
    }
}
...
fn resolve_requested_file(dist_dir: &Path, url_path: &str) -> PathBuf {
    ...
    if is_safe { candidate } else { dist_dir.join("index.html") }
}

fn handle_connection(stream: TcpStream, dist_dir: &Path) -> Result<()> {
    let mut reader = BufReader::new(stream.try_clone().context("Failed to clone connection stream")?);
...
fn write_response(stream: &mut TcpStream, code: u16, reason: &str, content_type: &str, body: &[u8]) -> Result<()> {
    let header = format!(

...
impl PlatformTarget for WindowsTarget {
    ...
    fn quick_reload(&self, ctx: &BuildContext) -> Result<bool> {
        let Some(hwnd) = find_root_window() else { return Ok(false) };
...
fn send_hot_swap(hwnd: isize, path: &Path) -> Result<()> {
    ...
    let path_str = path.to_str().context("windows dylib path is not valid UTF-8")?;
    ...
    unsafe {
        SendMessageW(hwnd as HWND, WM_COPYDATA, 0, &cds as *const COPYDATASTRUCT as isize);
    }
    Ok(())
}
